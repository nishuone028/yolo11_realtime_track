...
def main(config):
    ...
    # 只在多卡分布式训练时使用 DistributedDataParallel
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
...
if __name__ == '__main__':
    ...
    if use_distributed:
        ...
        rank = 0  # Set rank to 0 for single GPU mode
        world_size = 1  # Set world_size to 1 for single GPU mode
        torch.cuda.set_device(0)  # Assuming you are using the first GPU (index 0)

        seed = config.SEED  # Use the base seed for single GPU
        dist_rank = 0
    ...
    # logger
    logger = create_logger(output_dir=config.OUTPUT, dist_rank=dist_rank, name=f"{config.MODEL.ARCH}")
